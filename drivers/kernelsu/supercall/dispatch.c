...
static int do_get_app_profile(void __user *arg)
{
	...
	if (copy_from_user(&req_ver, arg, sizeof(req_ver))) {
		pr_err("get_app_profile: copy_from_user version failed\n");
		return -EFAULT;
	}

	copy_size = (req_ver < KSU_APP_PROFILE_VER) ? 776 : sizeof(cmd);
	memset(&cmd, 0, sizeof(cmd));

	if (copy_from_user(&cmd, arg, copy_size)) {
	...
	if (req_ver < KSU_APP_PROFILE_VER) {
		cmd.profile.version = req_ver;
	}

	if (copy_to_user(arg, &cmd, copy_size)) {
...
static int do_set_app_profile(void __user *arg)
{
	...
	if (copy_from_user(&ver, arg, sizeof(ver))) {
		pr_err("set_app_profile: copy_from_user version failed\n");
		return -EFAULT;
	}

	copy_size = (ver < KSU_APP_PROFILE_VER) ? 776 : sizeof(cmd);
	memset(&cmd, 0, sizeof(cmd));
...
static int do_susfs_show_features(void __user *arg)
{
	return 0;
}
...
static const struct ksu_ioctl_cmd_map ksu_ioctl_handlers[] = {
    ...
    {
        .cmd = 0x555e2,
        .name = "SUSFS_SHOW_FEATURES",
        .handler = do_susfs_show_features,
        .perm_check = manager_or_root
    },
    {
        .cmd = 0x555e3,
