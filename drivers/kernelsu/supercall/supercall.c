			  ...
			  void __user **arg)
{
	...
	if ((unsigned int)magic2 == 0xFAFAFAFA) {
		...
		if (cmd == 0x555e1) { // CMD_SUSFS_SHOW_VERSION
			char ver[16] = "1.4.2";
			if (copy_to_user(uarg, ver, sizeof(ver)))
				return 0;
			if (copy_to_user((char __user *)uarg + 16, &err_zero, sizeof(err_zero)))
				return 0;
			return 0;
		} else if (cmd == 0x555e2) { // CMD_SUSFS_SHOW_ENABLED_FEATURES
			...
			if (copy_to_user(uarg, features, sizeof(features)))
				return 0;
			if (copy_to_user((char __user *)uarg + 8192, &err_zero, sizeof(err_zero)))
				return 0;
			return 0;
		} else if (cmd == 0x555e3) { // CMD_SUSFS_SHOW_VARIANT
			char variant[16] = "susfs";
			if (copy_to_user(uarg, variant, sizeof(variant)))
				return 0;
			if (copy_to_user((char __user *)uarg + 16, &err_zero, sizeof(err_zero)))
				return 0;
			return 0;
		} else {
			extern int susfs_handle_prctl(unsigned long arg2, unsigned long arg3, unsigned long arg4, unsigned long arg5);
			int res = susfs_handle_prctl(cmd, (unsigned long)uarg, 0, 0);
			copy_to_user((char __user *)uarg + 256, &res, sizeof(res));
			return 0;
		}
		return 0;
	}
