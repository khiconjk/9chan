...
KBUILD_AFLAGS	+= -march=armv8.2-a+crypto
KBUILD_CFLAGS	+= -march=armv8.2-a+crypto
ifeq ($(CONFIG_SOC_EXYNOS9810), y)
KBUILD_CFLAGS	+= $(call cc-option,-mcpu=exynos-m3.cortex-a55+fp+simd+crc+lse+rdma+crypto,$(call cc-option,-mcpu=cortex-a75.cortex-a55+fp+simd+crc+lse+rdma+crypto,$(call cc-option,-mcpu=cortex-a55+fp+simd+crc+lse+rdma+crypto)))
KBUILD_CFLAGS	+= $(call cc-option,-mtune=exynos-m3.cortex-a55+fp+simd+crc+lse+rdma+crypto,$(call cc-option,-mtune=cortex-a75.cortex-a55+fp+simd+crc+lse+rdma+crypto,$(call cc-option,-mtune=exynos-m3+fp+simd+crc+lse+rdma+crypto)))
endif
...
KBUILD_CFLAGS   += -fgraphite-identity -floop-nest-optimize -floop-parallelize-all
endif
endif
